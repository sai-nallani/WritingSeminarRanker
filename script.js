...
function startMergeSort() {
    mergeQueue = courseNames.map(name => [name]); // Each course as a single-item array
    mergeResult = [];
    leftArr = [];
    rightArr = [];
    leftIdx = 0;
...
function nextMerge() {
    // If only one array left, we're done
    ...
    // If in the middle of a merge, continue
    if (leftArr.length && rightArr.length && (leftIdx < leftArr.length || rightIdx < rightArr.length)) {
        renderMergePair();
        return;
    }
    // Otherwise, pop two arrays to merge
    if (mergeQueue.length >= 2) {
        leftArr = mergeQueue.shift();
        rightArr = mergeQueue.shift();
        leftIdx = 0;
        rightIdx = 0;
        mergeResult = [];
        renderMergePair();
    }
    // If only one array left in queue, but not done, push it to the end for next round
    else if (mergeQueue.length === 1) {
        mergeQueue.push(mergeQueue.shift());
        nextMerge();
    }
}

function renderMergePair() {
    // If one side is exhausted, append the rest and push to queue
    if (leftIdx >= leftArr.length) {
        mergeResult = mergeResult.concat(rightArr.slice(rightIdx));
        mergeQueue.push(mergeResult);
        leftArr = [];
        rightArr = [];
        leftIdx = 0;
        rightIdx = 0;
    ...
    if (rightIdx >= rightArr.length) {
        mergeResult = mergeResult.concat(leftArr.slice(leftIdx));
        mergeQueue.push(mergeResult);
        leftArr = [];
        rightArr = [];
        leftIdx = 0;
    ...
    // Show the next pair to compare
    const leftName = leftArr[leftIdx];
    const rightName = rightArr[rightIdx];
    const container = document.getElementById('card-container');
    container.innerHTML = '';
    [leftName, rightName].forEach((name, i) => {
        const c = courses[name];
        const card = document.createElement('div');
        card.className = 'course-card';
        card.innerHTML = `
          <div class="course-title">${name}</div>
          <div class="course-instructor">${c.course_instructor}</div>
          <div class="course-timings">${c.course_timings.join('<br>')}</div>
          <div class="course-description">${c.course_description}</div>
          <div class="pick-label">Pick This</div>
        `;
        card.onclick = () => handleMergePick(i === 0 ? 'left' : 'right');
...
document.getElementById('show-ranking').onclick = function () {
    let final = mergeQueue[0];
    let html = '<h3 style="color:#e77500;">Your Ranking:</h3><ol>';
    final.forEach(name => {
        html += `<li><b>${name}</b> <span style="color:#888;font-size:0.95em;">(${courses[name].course_instructor})</span></li>`;
    });
    html += '</ol>';
...
undoBtn.style.margin = '20px auto 0 auto';
undoBtn.style.padding = '10px 24px';
undoBtn.style.background = '#fff';
undoBtn.style.color = '#e77500';
undoBtn.style.border = '2px solid #e77500';
undoBtn.style.borderRadius = '8px';
undoBtn.style.fontSize = '1em';
undoBtn.style.fontWeight = 'bold';
undoBtn.style.cursor = 'pointer';
undoBtn.style.boxShadow = '0 2px 8px #e7750033';
undoBtn.style.transition = 'background .18s';
undoBtn.onmouseover = () => undoBtn.style.background = '#fff8f0';
undoBtn.onmouseout = () => undoBtn.style.background = '#fff';
undoBtn.onclick = function () {
    ...
    deletedCourses = [...prev.deletedCourses];
    saveStateToStorage();
    if (rankingDone) {
        renderLiveRanking();
        document.getElementById('card-container').innerHTML = "<b>All done!</b>";
    ...
    document.getElementById('deleted-courses-list').innerHTML = deletedCourses.map(course => `<li>${course}</li>`).join('');
...
resetBtn.onclick = function () {
    if (confirm('Are you sure you want to reset your progress? This will erase your ranking and cannot be undone.')) {
        ...
        // Attempt to clear all localStorage and sessionStorage for this origin
        try {
            localStorage.clear();
            sessionStorage.clear();
        } catch (e) { }

        // Attempt to clear all cookies for this domain
...
function renderMergeSortVisualizer() {
    let vis = document.getElementById('merge-visualizer');
    if (!vis) {
        vis = document.createElement('div');
        vis.id = 'merge-visualizer';
        vis.style.marginTop = '36px';
        vis.style.padding = '18px 0 0 0';
        vis.style.borderTop = '2px solid #eee';
        document.getElementById('ranker-app').appendChild(vis);
    }
    ...
    if (leftArr.length && rightArr.length && (leftIdx < leftArr.length || rightIdx < rightArr.length)) {
        html += `<div style="margin-top:18px;">
            <b style="color:#e77500;">Currently Merging:</b>
            <div style="display:flex;gap:16px;margin-top:6px;">
                <div style="background:#e7750011;padding:6px 10px;border-radius:6px;">
                    <b>Left:</b> ${leftArr.map((n, idx) => idx === leftIdx ? `<u>${n}</u>` : n).join(', ')}
                </div>
                <div style="background:#e7750011;padding:6px 10px;border-radius:6px;">
                    <b>Right:</b> ${rightArr.map((n, idx) => idx === rightIdx ? `<u>${n}</u>` : n).join(', ')}
                </div>
                <div style="background:#e7750022;padding:6px 10px;border-radius:6px;">
                    <b>Merged:</b> ${mergeResult.join(', ')}
                </div>
            </div>
        </div>
        <div style="margin-top:18px;">
            <b style="color:#e77500;">Deleted Courses:</b>
            <div style="display:flex;gap:16px;margin-top:6px;" >
                <ul id="deleted-courses-list" style="list-style:none;margin:0;padding:0;">
                </ul>
            </div>
        </div>`;
    }
    vis.innerHTML = html;
    // Render deleted courses
    document.getElementById('deleted-courses-list').innerHTML = deletedCourses.map(course => `<li>${course}</li>`).join('');
}

// Call this at the end of renderMergePair and after undo:
function renderMergePair() {
    // If one side is exhausted, append the rest and push to queue
    if (leftIdx >= leftArr.length) {
        mergeResult = mergeResult.concat(rightArr.slice(rightIdx));
    ...
    [leftName, rightName].forEach((name, i) => {
        ...
        card.innerHTML = `
          ...
          <div class="course-description">${c.course_description}</div>
          <div class="pick-label">Pick This</div>
          <div class="pick-label delete-label" style="background:#dddddd;color:#e77500;font-weight:bold;cursor:pointer;">Delete Course</div>
        `;
        card.onclick = () => handleMergePick(i === 0 ? 'left' : 'right');
        container.appendChild(card);
    });
    ...
    document.querySelectorAll('div.delete-label').forEach(label => {
        label.onclick = function (event) {
            event.stopPropagation(); // Prevent card click
            const courseName = this.parentElement.querySelector('.course-title').innerText;
            if (confirm(`Are you sure you want to delete the course "${courseName}"?`)) {
                ...
                // Remove from mergeQueue
                mergeQueue = mergeQueue.map(arr => arr.filter(name => name !== courseName));
                // Remove from leftArr and rightArr if they contain the course
                leftArr = leftArr.filter(name => name !== courseName);
                rightArr = rightArr.filter(name => name !== courseName);
...
function loadStateFromStorage() {
    ...
    deletedCourses = (state.deletedCourses || []).slice();
    return true;
}
